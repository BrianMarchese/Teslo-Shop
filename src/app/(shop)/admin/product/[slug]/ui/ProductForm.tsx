...
export const ProductForm = ({ product, categories }: Props) => {
  ...
  // el watch sabe con que tiene que refrescar el ui
  ...
  const onSizeChanged = ( size: string ) => { // funcion para poder eliminar o agregar talles

    const sizes = new Set(getValues("sizes"))

    sizes.has( size ) ? sizes.delete(size) : sizes.add(size)

    setValue("sizes", Array.from(sizes)) // convierto los sizes en array y lo inserto en el value

  }

  const onSubmit = async( data: FormInputs ) => {
    
    const formData = new FormData() // crear un objeto con el formulario que quiero enviar
...

            {
              product.ProductImage?.map( image => (
                ...
                <div key={ image.id }>
                  ...
                  <ProductImage alt={ product.title! } src={ image.url } width={ 300 } height={ 300 } className="rounded-t shadow-md w-full" />

                  <button type="button" onClick={ () => deleteProductImage( image.id, image.url ) } className="btn-danger rounded-b-xl w-full">
                    Eliminar
                  </button>
